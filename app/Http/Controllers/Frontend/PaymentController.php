...
class PaymentController extends Controller
{
    ...
    {
        // 💡 جلب المعرف بالطريقتين: سيبحث عن invoice_id أولاً، وإذا كان فارغاً سيأخذ invId
        $invoiceId = $request->input('invoice_id') ?? $request->input('invId');

        // التحقق من وجود المعرف بأي من الاسمين
        if (empty($invoiceId)) {
            return back()->with('error', 'لم يتم تحديد فاتورة');
        }

        DB::transaction(function() use ($request, $invoiceId) {
            // استخدام المتغير المدمج المضمون
            ...
            // تحديث مبالغ الفاتورة وحالتها المالية
            ...
            // تحديث رصيد الخزنة الخاص بفرع الفاتورة وعملتها
