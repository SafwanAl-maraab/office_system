    ...
    <table class="w-full text-right border-collapse text-sm">
        ...
        @forelse($latestIncomes as $income)
            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-900/30 transition">
                ...
                <td class="p-3 text-xs max-w-xs truncate" title="{{ $income->description }}">
                    {{ $income->description }}
                </td>
                <td class="p-3 text-emerald-600 dark:text-emerald-400 font-bold font-mono">
                    +{{ number_format($income->amount, 2) }} {{ $income->currency->code ?? '' }}
                </td>
            </tr>
        @empty
            <tr>
                <td colspan="3" class="text-center p-8 text-gray-400 dark:text-gray-500 text-xs">
                    لا توجد إيرادات مسجلة مؤخراً.
                </td>
            </tr>
        @endforelse
        </tbody>
